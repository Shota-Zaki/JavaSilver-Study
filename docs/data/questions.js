window.JAVA_STUDY_DATA = {
  ...
  "version": "20260702-code-audit-v37-ch07-q35-q60-alignment-garbled-text-fix",
  "chapters": [
    {
      "id": "ch01",
  ...
  "questions": {
    ...
    "ch07": [
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問35 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\n`indexOf(\"ef\")` は、文字列 `\"abcd ef gh\"` の中で `\"ef\"` が始まる位置を返します。インデックスは0から数えるため、戻り値は5です。\n\n次の `str.substring(x + 3)` は `substring(8)` となり、結果として `\"gh\"` を返します。ただし、`String` は不変オブジェクトです。`substring` は元の文字列を変更せず、新しい文字列を戻します。この戻り値を変数へ代入していないため、`str` は `\"abcd ef gh\"` のままです。\n\nその後にもう一度 `indexOf(\"ef\")` を実行しても、`x` は5になります。したがって、表示結果は次のとおりです。\n\n```text\nabcd ef gh 5\n```\n\nよって、選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問36 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\nクラス変数 `str` は明示的に初期化されていないため、参照型フィールドのデフォルト値である `null` で初期化されます。\n\n`switch` の条件式に `String` を指定することはできますが、その値が `null` の場合は分岐処理を開始できず、`NullPointerException` がスローされます。例外は最初の `case` が評価される前に発生するため、`str += \"def\"` や `str += \"20\"` は実行されず、`println` にも到達しません。\n\nしたがって、実行時に例外がスローされる選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問37 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：B\n\n両クラスは `ex15` パッケージに属しています。`javac -d build ...` を実行すると、パッケージ構造に対応した次のクラスファイルが `build` 配下へ生成されます。\n\n```text\nbuild/ex15/Sample.class\nbuild/ex15/Main.class\n```\n\n実行時は、パッケージ階層の起点である `build` をクラスパスに指定し、完全修飾クラス名 `ex15.Main` を渡します。\n\n```text\njava -cp build ex15.Main\n```\n\n選択肢Aはクラスファイルを `build` に出力していないのに、実行時のクラスパスを `build` にしています。選択肢Cは `build` に出力しているのにクラスパスを指定していません。選択肢Dの `build.ex15.Main` は、ディレクトリ名をパッケージ名の一部として扱っているため誤りです。\n\nよって、選択肢Bが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問38 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\n`new B()` で生成される実体は `B` ですが、変数 `a` の宣言型は `A` です。\n\n```java\nA a = new B();\n```\n\nメソッド呼び出しがコンパイルできるかどうかは、まず変数の宣言型を基準に判定されます。`A` には `hello()` しか定義されておらず、`bye()` は `B` 独自のメソッドです。そのため、`a.bye()` に対応するメソッドを `A` から見つけられず、コンパイルエラーになります。\n\nオーバーライドによる動的ディスパッチは、宣言型から呼び出せるメソッドが確定した後に働く仕組みです。宣言型に存在しないメソッドを呼び出せるようにはなりません。\n\nよって、選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問39 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\n`Sample` クラスに定義されている `test` メソッドは、`char` と `int` の2つの引数を必要とします。\n\n```java\nString test(char a, int b)\n```\n\n一方、呼び出し側が渡しているのは `'A'` だけです。\n\n```java\napp.test('A')\n```\n\nJavaでは、フィールド `b` が存在していても、足りないメソッド引数を自動的に補うことはありません。`test(char)` というオーバーロードも定義されていないため、呼び出せるメソッドが見つからずコンパイルエラーになります。\n\nよって、選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問40 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A、E\n\n配列型の `[]` の数と、生成する配列の次元数が一致しているかを確認します。\n\n選択肢Aは、2次元配列型 `int[][]` に2次元の配列初期化子を代入しているため正しいコードです。\n\n選択肢Bは変数が3次元配列型ですが、初期化子は2次元です。選択肢Cは変数が2次元配列型ですが、初期化子は1次元です。どちらも次元数が一致しません。\n\n選択肢Dの `new int[][2]` は、配列生成時に先頭の次元を省略しているためコンパイルエラーになります。`new` で多次元配列を生成するときは、少なくとも最初の次元の要素数が必要です。\n\n選択肢Eでは、`array2[0][0]` などの型は `int[]` です。そのため、`int[]` 型の変数 `array` を代入できます。\n\nよって、選択肢AとEが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問41 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A\n\n`A`、`B`、`C` は同じ無名パッケージに属しています。`A.num` にはアクセス修飾子がないため、同じパッケージ内からアクセスできます。\n\n`C` の `getValue()` 内にある `super.num` は、スーパークラス側から継承した `num` フィールドを参照します。また、`super.getValue()` は直接のスーパークラスである `B` の実装を呼び出します。\n\nしたがって、`B` のサブクラスである `C` から、`getValue` メソッド内で `A` 由来のフィールドへアクセスできます。選択肢Bはフィールドへアクセスできないとしている点が誤りです。`new.getValue()` と `public.getValue()` という構文は存在しないため、選択肢CとDも誤りです。\n\nよって、選択肢Aが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問42 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A、C、E\n\nこの問題では、オーバーロードの選択とオーバーライドの実行を分けて考えます。オーバーロードはコンパイル時の変数型と引数型で決まり、オーバーライドは実行時のインスタンス型で決まります。\n\n- `a1.sample(list)`：`a1` は `A` 型で実体も `A` です。`sample(Collection)` が実行され、`A` と表示されます。\n- `a2.sample(list)`：`a2` の宣言型は `A` なので、コンパイル時には `sample(Collection)` が選ばれます。実体は `B` であり、そのメソッドがオーバーライドされているため、`B` と表示されます。\n- `b1.sample(list)`：`b1` は `B` 型で、引数は `List` 型です。`sample(Collection)` より具体的な `sample(List)` が選ばれ、`C` と表示されます。\n\nよって、選択肢A、C、Eが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問43 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\n`B` と `C` は、どちらも `A` を継承し、同じシグニチャのデフォルトメソッド `sample()` を定義しています。`D` が両方を実装すると、どちらのデフォルト実装を使うか決められないため、`D` 自身で `sample()` をオーバーライドして競合を解消する必要があります。\n\n設問ではオーバーライド自体は記述されていますが、メソッド本体の次の呼び出しが不正です。\n\n```java\nsuper.sample();\n```\n\nインタフェースのデフォルトメソッドを明示的に呼ぶ場合は、次のように対象のインタフェース名を指定します。\n\n```java\nB.super.sample();\n// または\nC.super.sample();\n```\n\nそのため、設問の `D` クラスはコンパイルエラーになります。よって、選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問44 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\nクラス関係は `D extends C extends A` であり、`C` は `B` インタフェースを実装しています。したがって、`D` も `A` のサブタイプであり、`B` の実装型です。\n\n- A：`D` は `A` のサブタイプなので追加できます。\n- B：`C` は `B` を実装しているため追加できます。\n- C：`D` は `C` を継承し、`B` も実装しているため追加できます。\n- D：`C` は `D` のスーパークラスです。スーパークラスのインスタンスを `List<D>` へ追加することはできません。\n- E：`C` は `A` のサブタイプなので追加できます。\n\nよって、コンパイルエラーになる選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問45 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A、B、F\n\n`A` はインタフェース、`B` は `A` を実装するクラスです。\n\n- A：インタフェースを `final` にすると継承できなくなり、インタフェースの目的と矛盾するため指定できません。正しい説明です。\n- B：実装済みのクラスであっても、クラスを `abstract` にすることはできます。正しい説明です。\n- C：`B` を `final` クラスにすることは可能です。誤りです。\n- D：`A` が `B` のサブタイプなのではありません。誤りです。\n- E：インタフェースは暗黙的に抽象であり、`abstract interface A` と明示することもできます。誤りです。\n- F：`B` は `A` を実装しているため、`A` のサブタイプです。正しい説明です。\n\nなお、`build` メソッドの戻り値を `A` から `B` に変更してオーバーライドできるのは、`B` が `A` のサブタイプであるためです。これは共変戻り値です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問46 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：C\n\n`new C()` を実行すると、まずスーパークラス側の初期化が必要です。各コンストラクタの先頭には、明示されていない `super()` が補われます。\n\n呼び出し順は次のとおりです。\n\n1. `C()` から `B()` を呼ぶ。\n2. `B()` から `A()` を呼ぶ。\n3. `A()` が `A` を表示する。\n4. `B()` に戻り、`B` を表示する。\n5. `C()` に戻り、`C` を表示する。\n\nしたがって、表示結果は `ABC` です。よって、選択肢Cが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問47 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A\n\n`A.print()` と `B.print()` はどちらも `private` です。`private` メソッドはサブクラスへ継承されないため、`B.print()` は `A.print()` をオーバーライドしているのではなく、別のメソッドとして定義されています。\n\n`b.a()` は `A` から継承した `a()` を実行します。`a()` 内の `print()` は `A.print()` を指すため、`A` と表示されます。\n\n続く `b.b()` は `B.b()` を実行し、その中の `print()` は `B.print()` を指すため、`B` と表示されます。\n\n```text\nA\nB\n```\n\nよって、選択肢Aが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問48 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：C、E\n\n戻り値型と `return` 文の値が一致しているかを確認します。\n\n- A：戻り値型が `void` なのに、`return ++x;` で値を返しているため誤りです。\n- B：戻り値型が `String` なのに、正常終了する経路で値を返していないため誤りです。\n- C：比較式 `a < 0` の結果は `boolean` なので正しい定義です。\n- D：戻り値型は `char` ですが、返している `str` は `String` なので誤りです。\n- E：`void` メソッドでは、値を伴わない `return;` を使って処理を終了できます。正しい定義です。\n\nよって、選択肢CとEが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問49 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：B\n\n実行コマンドは次のとおりです。\n\n```text\njava A \"AB\" AB\n```\n\n二重引用符は、空白を含む文字列などを1つの起動パラメータとして扱うためにシェルが使用する記号です。引用符そのものは `args` の文字列には含まれません。\n\nしたがって、`args` の内容は次の2要素です。\n\n```text\nargs[0] = \"AB\"\nargs[1] = \"AB\"\n```\n\n拡張for文で順番に改行なしで表示するため、結果は `ABAB` になります。よって、選択肢Bが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問50 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：C\n\nメソッドをオーバーライドするときは、元の戻り値型と同じ型、またはそのサブタイプを戻り値型にできます。これを共変戻り値といいます。\n\n`B.a()` の戻り値型 `Collection` は `Iterable` のサブタイプなので、`A.a()` を正しくオーバーライドしています。`C.a()` の戻り値型 `Path` も `Iterable<Path>` を継承しているため、`Iterable` のサブタイプとして扱えます。\n\nしかし、`D` が `B` と `C` を同時に継承すると、同じシグニチャの `a()` について、戻り値型が `Collection` と `Path` の2種類になります。`Collection` と `Path` の間には一方が他方のサブタイプとなる関係がないため、1つのメソッドとして両方を満たせません。\n\nよって、BとCから継承したメソッドの戻り値型が互いに両立しないことを示す選択肢Cが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問51 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：C\n\nフィールドはメソッドと異なり、実行時のインスタンス型による動的ディスパッチの対象ではありません。どのフィールドを参照するかは、変数の宣言型で決まります。\n\n```java\nA a = new B();\nSystem.out.println(a.x);\n```\n\n変数 `a` の宣言型は `A` なので、参照されるのは `A.x` です。`A.x` は `public` で値は `\"A\"` であるため、`A` と表示されます。\n\n`B` に同名のフィールド `x` がありますが、これはオーバーライドではなくフィールドの隠蔽です。また、`B.x` はパッケージプライベートなので、別パッケージの `Main` から直接参照することもできません。\n\nよって、選択肢Cが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問52 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：E\n\n`SubSample(int price)` の先頭には、コンパイラによって暗黙の `super()` が追加されます。しかし、スーパークラス `Sample` には `Sample(String, int)` しかなく、引数なしコンストラクタは存在しません。そのため、1つ目のコンストラクタでコンパイルエラーになります。\n\n```java\npublic SubSample(int price) {\n    // 暗黙に super(); が必要だが、Sample() は存在しない\n    this.price = price;\n}\n```\n\n2つ目のコンストラクタでは、`super(name, num);` の後に `this(price);` を記述しています。`super(...)` または `this(...)` による別コンストラクタの呼び出しは、コンストラクタの先頭文に1つだけ記述できます。両方を同じコンストラクタから呼ぶことはできません。\n\nしたがって、設問で示された3行目と8行目の両方がコンパイルエラーとなり、選択肢Eが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問53 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：E\n\n`B` は `A` を継承しているため、抽象メソッド `sample()` と `test()` の両方を持ちます。\n\n抽象クラス `C` は `test()` を実装していますが、`sample()` は実装していません。`C` 自身が抽象クラスなので、未実装の抽象メソッドを残すことは可能です。\n\n一方、`D` は具象クラスです。具象クラスは継承したすべての抽象メソッドを実装しなければなりません。`D` は `test()` を再定義しているだけで、`sample()` を実装していないためコンパイルエラーになります。\n\nよって、選択肢Eが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問54 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A、C\n\n例外処理の構文を確認します。\n\n- A：`finally` は、`catch` がある場合は最後の `catch` の直後、`catch` がない場合は `try` の直後に置きます。正しい説明です。\n- B：try-with-resourcesでは、リソースを閉じた後に `catch` や `finally` へ進みます。誤りです。\n- C：1つの `try` に複数の `catch` を続けて記述できます。正しい説明です。\n- D：継承関係にある例外を複数捕捉するときは、サブクラス型からスーパークラス型の順に並べます。一般的な型を先にすると、後続の具体的な型が到達不能になります。誤りです。\n- E：通常の `try` には `catch` または `finally` の少なくとも一方が必要ですが、両方を必ず記述する必要はありません。誤りです。\n\nよって、選択肢AとCが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問55 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：C\n\n`RuntimeException` は `Exception` のサブクラスです。最初の `catch (Exception e)` は、`RuntimeException` を含むすべての `Exception` 系例外を捕捉できます。\n\nそのため、後ろにある次の `catch` へ処理が到達する可能性はありません。\n\n```java\ncatch (RuntimeException e) {\n    System.out.println(\"A\");\n}\n```\n\n到達不能な `catch` ブロックはコンパイルエラーになります。実行時の表示や `finally` の動作を追う前に、コンパイルが失敗します。\n\nよって、選択肢Cが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問56 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A\n\n`==` は、`String` の文字内容ではなく、同じオブジェクトを参照しているかを比較します。\n\n- `s1` は `new String(\"Java\")` により、新しい `String` オブジェクトを参照します。\n- `s2` は文字列リテラルなので、文字列プール内の `\"Java\"` を参照します。\n- `s3` は `s2.intern()` の戻り値です。`s2` 自体が文字列プール内のオブジェクトを参照しているため、`s3` も `s2` と同じオブジェクトを参照します。\n\nしたがって、比較結果は次のとおりです。\n\n```text\ns1 == s2  -> false\ns2 == s3  -> true\ns1 == s3  -> false\n```\n\nよって、`false, true, false` と表示される選択肢Aが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問57 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A\n\n2次元配列 `arrays` の各要素は、次の1次元 `char` 配列です。\n\n```text\n{'a', 'd'}\n{'b', 'e'}\n{'c', 'f'}\n```\n\n外側の拡張for文は1行分の `char[]` を順番に取り出します。内側の拡張for文は、その配列内の文字を改行せずに表示します。各行を表示した後に空白を1つ出力します。\n\nしたがって、表示順は `ad`、`be`、`cf` となり、結果は次のとおりです。\n\n```text\nad be cf \n```\n\n末尾にも空白がありますが、選択肢上の表示内容としてはAが該当します。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問58 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：A\n\n`new B()` からコンストラクタの呼び出しを追います。\n\n1. `B()` は最初に `this(4)` を呼び、`B(int)` へ移ります。\n2. `B(int)` の先頭には暗黙の `super()` が追加され、`A()` を呼びます。\n3. `A()` は `this(2)` を呼び、`A(int)` が `2` を表示します。\n4. `A()` に戻り、`1` を表示します。\n5. `B(int)` に戻り、引数 `4` を表示します。\n6. `B()` に戻り、`3` を表示します。\n\nしたがって、表示結果は `2143` です。よって、選択肢Aが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
      ...
      },
      {
        ...
        "explanation": {
          "summary": "第7章 問59 解説（重複PDFずれ・文字化け修正版）。",
          "pdfExplanation": "正解：D\n\n配列の内容は次のとおりです。\n\n```text\narray[0][0] = \"a\"\narray[0][1] = \"b\"\narray[1][0] = \"c\"\narray[1][1] = \"d\"\n```\n\n選択肢Dは、外側の `i` と内側の `j` をそれぞれ0から1まで進めます。順番に `[0][0]`、`[0][1]`、`[1][0]`、`[1][1]` を参照するため、`abcd` と表示されます。\n\n選択肢Aは `[1][1]` の `d` だけを表示します。選択肢Bは `i = 1` のときの `[1][0]`、つまり `c` だけを表示します。選択肢Cは、2次元配列の要素型が `String[]` なのに外側の変数を `String` としているためコンパイルエラーです。\n\nよって、選択肢Dが正解です。\n",
          "pdfExplanationOnly": true,
          "pdfAlignmentNote": "問35・問36の重複PDFに起因する問37以降の1問ずれを補正。問題文・コード・選択肢・正答との整合を優先して解説を再構成し、OCR由来の文字化けと混入断片を除去。元PDF画像との1文字単位の逐字照合は未実施。"
        },
        "source": "",
